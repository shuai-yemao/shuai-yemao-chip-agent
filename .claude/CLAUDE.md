...
对任意任务需求，先跑需求对齐层。输出中会自动包含 `chain` 元数据，引导后续步骤：
...
输出 Guide + 4 张清单模板 + **链式触发指引**（`chain.flow.next_conversations`）。

主 agent 按 chain 指引在对话中逐步完成：

1. **锚定方向** → 调 `grill-me` 技能，逐条问答确认（chain step 1）
2. **约束梳理** → 调 `grill-with-docs` + `to-prd`，确认约束和 PRD（chain step 2）
3. **六源对齐审查** → Skills + WebSearch + GitHub + KB(记忆层) + 真伪验证（chain step 3）

   需求对齐层 Phase 0.5 已自动检索记忆层中的历史经验注入分析（`kb_context`），
   审查时主 agent 可以直接使用，也建议用 `memory-layer search` 补充检索。

4. **清单确认** → 逐条确认 4 张施工清单（现状/约束/文件/验收）（chain step 4）
5. **拆解执行** → 调 `to-issues` 拆解 → 按 `chain.flow.next_workflow` 传到编排层

清单全部确认后，chain 指引自动告诉 next_workflow 的参数映射：
